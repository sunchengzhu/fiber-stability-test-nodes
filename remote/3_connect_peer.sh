...
for idx in "${!INTERNAL_PORTS[@]}"; do
  ...
  address=$(echo "$response" | jq -r '.result.addresses[]' | sed "s/127.0.0.1/$IP/" | grep -v "/ws/" | head -n 1)
  addresses+=("$address")
done
...
if [ "$current_ip" == "18.167.71.41" ]; then
  ...
  curl -s --location "http://172.31.28.209:$port" --header "Content-Type: application/json" --data "$json_data"
  echo ""
  ...
  curl -s --location "http://172.31.16.223:$port2" --header "Content-Type: application/json" --data "$json_data2"
  echo ""
fi
